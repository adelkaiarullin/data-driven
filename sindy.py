...
def compute_linear_model(x, y, z, u, delta):
    ...
    dmatrix = np.vstack([np.ones_like(c_x), c_x, c_y, c_z,
        c_x * c_y, c_y * c_z, c_x * c_z, c_x ** 2, c_y ** 2, c_z ** 2,
        c_x**3, c_y**3, c_z**3,
        u_x, u_x ** 2, u_y, u_y ** 2, u_z, u_z ** 2])

    model = ['1', 'x', 'y', 'z', 'x*y', 'y*z', 'x*z', 'x^2', 'y^2', 'z^2',
             'x^3', 'y^3', 'z^3',
             'ux', 'ux^2', 'uy', 'uy^2', 'uz', 'uz^2']
...
def compute_sindy_for_lorenz(x, y, z, u, delta):
    clf = MultiOutputRegressor(linear_model.Lasso(alpha=0.1, max_iter=5000))
    c_x, c_y, c_z = x[:-1], y[:-1], z[:-1]
    u_x, u_y, u_z = u[:-1, 0], u[:-1, 1], u[:-1, 2]
    p_x, p_y, p_z = x[1:], y[1:], z[1:]
...
if __name__ == '__main__':
    ...
    compute_linear_model(data[:,0], data[:,1], data[:,2], u, delta)
